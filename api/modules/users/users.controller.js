...
const editUser = async (req, res) => {
  ...
  if (!id) {
    return res
        ...
        .json({
          success: 0,
          message: usersConstants.responseMessages.editUser.noId,
          data: null,
        });
  }

  logger.info('Editing single user data with id: ', { id: id });
  ...
  if (err) {
    logger.error('Could not edit user', { error: err }, { userId: id });
    return res.status(statusCodes.internalError).json({
      success: 0,
      message: usersConstants.responseMessages.editUser.failure,
      data: null,
    });
  }
  ...
  return res.status(statusCodes.success).json({
    success: 1,
    message: usersConstants.responseMessages.editUser.success,
    data: user,
  });
};
